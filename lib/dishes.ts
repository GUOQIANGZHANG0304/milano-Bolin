...
export type DishRecord = { id: string; name: string; nameIt?: string; category: string; price: number; image: string; description: string; descriptionIt?: string; tags: string[]; isPublished: boolean; featured?: boolean };
...
export function normalizeDbDish(row:{id:number;name:string;nameIt:string;category:string;price:number;image:string;description:string;descriptionIt:string;tags:string[];isPublished:boolean;isFeatured:boolean}):DishRecord{return{...row,id:String(row.id),category:legacyCategoryMap[row.category]??row.category,tags:Array.isArray(row.tags)?row.tags:[],featured:row.isFeatured}}
