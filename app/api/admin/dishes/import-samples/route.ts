...
export async function POST(request: Request) {
  ...
  try {
    ...
    const imported = await db.insert(dishes).values(missingSamples.map(({ name, category, price, image, description, isPublished }) => ({
      name, category, price, image, description, isPublished,
