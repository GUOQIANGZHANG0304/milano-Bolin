...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const descriptionIt = String(body.descriptionIt ?? "").trim();
    ...
    if (!name || !nameIt || !category || !image || !description || !descriptionIt || !Number.isFinite(price) || price <= 0) {
      return Response.json({ error: "请完整填写中意双语菜品信息，价格需大于 0。" }, { status: 400 });
    }
    const db = getDb();
    const [previous] = await db.select({ image: dishes.image }).from(dishes).where(eq(dishes.id, id)).limit(1);
    const [dish] = await db.update(dishes).set({ name, nameIt, category, image, description, descriptionIt, price, tags, isPublished, isFeatured }).where(eq(dishes.id, id)).returning();
