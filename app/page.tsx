...
export default async function Home(){let featured=sampleDishes.slice(0,3);try{const rows=await getDb().select().from(dishes).where(eq(dishes.isPublished,true)).orderBy(desc(dishes.id)).limit(3);featured=rows.map(normalizeDbDish)}catch{}return <main><SiteHeader/>
