...
<section className="section" id="featured"><div className="section-heading"><div><span className="eyebrow">本店甄选</span><h2>今日招牌</h2></div><Link href="/menu" className="text-link">查看全部菜品 <ArrowRight size={17}/></Link></div><div className="dish-grid">{sampleDishes.slice(0,3).map((dish,index)=><DishCard key={dish.id} dish={dish} index={index}/>)}</div></section>
