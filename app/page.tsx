...
<section className="section" id="featured"><div className="section-heading"><div><span className="eyebrow">本店甄选</span><h2>今日招牌</h2></div><a href="/menu" className="text-link">查看全部菜品 <ArrowRight size={17}/></a></div>{featured.length?<div className="dish-grid">{featured.map((dish,index)=><DishCard key={dish.id} dish={dish} index={index}/>)}</div>:<div className="status-card">今日菜品正在更新，请稍后再来看看。</div>}</section>
