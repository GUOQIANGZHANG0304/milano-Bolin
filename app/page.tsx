...
export default async function Home(){let featured=sampleDishes.filter(dish=>dish.featured);try{const rows=await getDb().select().from(dishes).where(and(eq(dishes.isPublished,true),eq(dishes.isFeatured,true))).orderBy(desc(dishes.id));featured=rows.map(normalizeDbDish)}catch{}return <main><SiteHeader/>
<section className="hero" id="about"><img className="hero-bg" src="/hero-feast.png" alt="米兰博林外卖打包点心店招牌菜"/><div className="hero-shade"/><div className="hero-content"><span className="eyebrow light">米兰点心 · 现做外带</span><h1>一席好味，<br/>打包带走。</h1><p>米兰博林外卖打包点心店，为附近街坊准备现做中式点心与家常菜。提前选好，轻松到店取餐。</p><div className="hero-actions"><a href="/menu" className="button primary">浏览完整菜单 <ArrowRight size={18}/></a><a href="#contact" className="button ghost">查看门店信息</a></div></div><div className="hero-note"><Sparkles size={18}/><span><b>今日推荐</b>每日菜品，随到店食材更新</span></div></section>
...
<section className="section" id="featured"><div className="section-heading"><div><span className="eyebrow">本店甄选</span><h2>今日招牌</h2></div><a href="/menu" className="text-link">查看全部菜品 <ArrowRight size={17}/></a></div><FeaturedCarousel dishes={featured}/></section>
